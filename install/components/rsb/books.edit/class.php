...
use RSB\Books\Helpers;
use RSB\Books\ORM\BooksTable;

if (!defined("B_PROLOG_INCLUDED") || B_PROLOG_INCLUDED !== true) die();

class BooksEditComponent extends CBitrixComponent implements Controllerable
{
    private $fieldsMap;
    private $entityID;
    ...
    public function prepareEntityData()
    {
        ...
        $this->entityData = BooksTable::query()
            ->addSelect('*')
    ...
    public function saveAction(): ?array
    {
        ...
        foreach (BooksTable::getFieldsInfo() as $field) {
            if ($field['IS_PRIMARY'])
                continue;
            ...
            $fields[$field['NAME']] = is_array($data[$field['NAME']]) ? $data[$field['NAME']][0] : $data[$field['NAME']];
        }
        ...
        if ($isNew) {
            $result = BooksTable::add(['fields' => $fields]);
        } else {
            $result = BooksTable::update($ID, $fields);
        }
